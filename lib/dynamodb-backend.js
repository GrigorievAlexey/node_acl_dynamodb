...
DynamoDBBackend.prototype = {
    ...
    */
    get: function(bucket, key, cb) {
        ...
        this.client.get(params, function(err, data) {
            if (err) {
                if (err.code === 'ResourceNotFoundException') return cb(null, []);
                return cb(err);
            } 
    ...
    union: function(bucket, keys, cb) {
        ...
        function tryUntilSuccess(params, result, retryCount, innercb) {
            self.client.batchGet(params, function(err, data) {
                if (err) {
                    if (err.code === 'ResourceNotFoundException') return innercb(null, []);
                    return innercb(err);
                } 
    ...
    */
    remove: function(transaction, bucket, key, values) {
        ...
        params.ExpressionAttributeNames = mapToExpressionAttributeNames(makeArray(values));
        values = Object.keys(params.ExpressionAttributeNames);
...
function mapToExpressionAttributeNames(attributes) {
    return attributes.reduce(function (acc, item, i) {
        acc['#item' + i] = item;
        return acc;
    }, {});
}
